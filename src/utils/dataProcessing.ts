...
export function processRawData(rawData: RawDataRow[]): ProcessedData[] {
  ...
  // First, create individual entries for each class occurrence
  ...
  rawData.forEach((row, index) => {
    try {
      ...
      const uniqueID = `${cleanedClass}-${dayOfWeek}-${classTime}-${location}-${dateOnly}-${index}`.replace(/\s+/g, '_');
      
      // Create individual class record
      ...
      // Add to individual classes array
      individualClasses.push(classRecord);

      // Now group data for aggregation

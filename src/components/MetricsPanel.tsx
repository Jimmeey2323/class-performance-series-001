...
import { LineChart, Clock, Calendar, User, Percent, DollarSign, Activity, BarChart3, Users, CheckCircle2, XCircle, BarChart } from 'lucide-react';
...
const generateSparklineData = (data: ProcessedData[], field: keyof ProcessedData, periods: number = 10): number[] => {
  ...
  const periodData = data.reduce((acc: Record<string, number>, item) => {
    ...
    const value = typeof item[field] === 'number' ? item[field] as number : typeof item[field] === 'string' ? parseFloat(item[field] as string) || 0 : 0;
    acc[period] += value;
    return acc;
  }, {});
            ...
            <Card className="h-28 border shadow-sm overflow-hidden">
              <CardContent className="p-3 h-full flex flex-col">
                ...
                <div className="mt-1 text-lg font-semibold">
                  {typeof metric.value === 'number' ? showCountUp ? <CountUp start={0} end={metric.value} decimals={metric.title.includes('Avg') || metric.title.includes('Rate') ? 1 : 0} separator="," decimal="." /> : 0 : metric.value}
                </div>
                <div className="mt-auto h-8">
                  {metric.sparkData && metric.sparkData.length > 1 && <Sparklines data={metric.sparkData} height={20} margin={0}>
                      <SparklinesLine color={metric.textColor.replace('text-', '')} style={{
                  strokeWidth: 2,
                  fill: "none"
                }} />
                      <SparklinesSpots size={1} style={{
